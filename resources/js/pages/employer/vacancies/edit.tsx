...
export default function EditVacancyPage({vacancy}:{vacancy:Vacancy}) {
    const { data, setData, patch, processing, errors } = useForm<Vacancy>(vacancy);

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        patch(route("vacancies.patch",vacancy.id), {
            onSuccess: () => {
    ...
    return (
        <>
            <AppLayout breadcrumbs={breadcrumbs}>
                ...
                <h1 className="mt-4 ml-8 text-3xl font-bold">Редактировать вакансию</h1>
